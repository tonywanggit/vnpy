...
class BollingerBotP2001Strategy(CtaInvestmentTemplate):
    """基于布林通道的交易策略"""
    ...
    fixWinPrcnt = 2.6   # 固定止盈

    # （多头参数）
    bollLength = 22  # 通道窗口数
    entryDev = 2.0  # 开仓偏差
    exitDev = 0.8  # 平仓偏差
    trailingPrcnt = 2.0  # 移动止损百分比
    maLength = 24  # 过滤用均线窗口

    # （空头参数）
    shortBollLength = 14  # 通道窗口数
    shortEntryDev = 3.6  # 开仓偏差
    shortExitDev = 2.1  # 平仓偏差
    shortTrailingPrcnt = 1.5  # 移动止损百分比
    shortMaLength = 16  # 过滤用均线窗口
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        """"""
        super(BollingerBotP2001Strategy, self).__init__(
            cta_engine, strategy_name, vt_symbol, setting
        )

        self.bg = BarGenerator(self.on_bar, 3, self.on_xmin_bar)
