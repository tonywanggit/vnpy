...
class CincoYStrategy(CtaTemplate):
    ...
    def on_xmin_bar(self, bar: BarData):
        ...
        if not self.pos:
            ...
            self.long_percent_stop = self.intra_trade_high * (1 - self.stop_long_percent / 100)

            self.sell(max(self.long_stop, self.long_percent_stop), abs(self.pos), stop=True)
        
        else:
            self.intra_trade_low = min(self.intra_trade_low, bar.low_price)
            self.short_stop = self.intra_trade_low + self.trailing_short * boll_width
            self.short_percent_stop = self.intra_trade_low * (1 + self.stop_short_percent / 100)
            
            self.cover(min(self.short_stop, self.short_percent_stop), abs(self.pos), stop=True)
